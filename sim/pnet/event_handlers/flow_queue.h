...
public:
    ...
    FlowQueue(EventList& eventlist, const std::string &name, StopLogger* stop_logger, size_t concurrency = std::numeric_limits<size_t>::max(), bool loop_flow = false);
    ...
    void doNextEvent(EventSource *parent) override;
...
private:
    ...
    // Restart the flow that just finished.
    void RestartFlow(EventSource *flow_src);
};
