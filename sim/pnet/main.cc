...
struct program_argument {
    ...
    std::string flow_size_distribution_filename;
...
#define VALIDATE_ARGS_IF(condition, assertion, ...) { \
    ...
    static struct option long_options[] = {
            ...
            { "flow_size_distribution", required_argument, nullptr, static_cast<int>(LONG_ARGUMENT::FLOW_SIZE_DISTRIBUTION) },
            ...
            { "loop_flows", no_argument, nullptr, static_cast<int>(LONG_ARGUMENT::LOOP_FLOWS) },
        ...
        switch (c) {
            ...
            case static_cast<int>(LONG_ARGUMENT::FLOW_SIZE):
                if (sscanf(optarg, "%zu", &(args.flow_size)) != 1)
                    goto invalid;
                break;
            case static_cast<int>(LONG_ARGUMENT::FLOW_SIZE_DISTRIBUTION):
                args.flow_size_distribution_filename = std::string(optarg);
                break;
            ...
            case static_cast<int>(LONG_ARGUMENT::QUEUE_SIZE_MULTIPLIER):
                if (sscanf(optarg, "%zu", &(args.queue_size_multiplier)) != 1)
                    goto invalid;
                break;
            case static_cast<int>(LONG_ARGUMENT::LOOP_FLOWS):
                args.loop_flows = true;
                break;
            default:
    ...
    if (args.traffic_type != TrafficType::FILE) {
        ...
        VALIDATE_ARGS_IF(!args.flow_size_distribution_filename.empty(), args.traffic_mode == TrafficMode::SHORT, "Cannot specify flow szie distribution for non-short flows.");
        VALIDATE_ARGS_IF(args.loop_flows, args.traffic_mode == TrafficMode::SHORT, "Can only loop flows for short flows.");
...
int main(int argc, char *argv[]) {
    ...
    for (const auto& traffic_matrix_file: args.traffic_matrix_files) {
        ...
        if (args.traffic_type != TrafficType::FILE) {
            ...
            if (args.traffic_mode == TrafficMode::SHORT) {
                CHECK(args.flow_size > 0 ^ !args.flow_size_distribution_filename.empty(), "Must specify either flow_size or distribution for short flows.");
                auto *flow_size_distribution = new CDFDistribution<int>(args.flow_size_distribution_filename);
                traffic_matrix->SetFlowSize(args.flow_size, flow_size_distribution);
                if (args.loop_flows)
                    traffic_matrix->SetLoopFlows();
            }
            if (args.flow_rate != 0)
