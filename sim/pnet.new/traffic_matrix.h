...
private:
    ...
    CDFDistribution<int> *flow_size_distribution;
    ...
    void LogFlowSizes(const Flowsets & flowsets);
...
public:
    ...
    inline void SetEndTime(double end_time_ms) {
        ...
        if ((this->trafficMode == TrafficMode::SHORT && !this->loop_flows) || this->trafficMode == TrafficMode::RPC)
            throw std::runtime_error("Cannot set end time for non-loop short flows.");
    ...
    inline void SetFlowSize(size_t size_in_bytes, CDFDistribution<int> *cdf_distribution = nullptr) {
        CHECK(size_in_bytes > 0 ^ cdf_distribution != nullptr, "Can only specify either size_in_bytes or distribution");
        ...
        this->flow_size_distribution = cdf_distribution;
        std::cerr << "Flow size set to " << this->flow_size << std::endl;
        std::cout << "TM[" << this->tm_id << "] flow_size = " << this->flow_size << std::endl;
        if (this->flow_size_distribution != nullptr)
            std::cout << "TM[" << this->tm_id << "] flow_size_distribution = " << this->flow_size_distribution->GetDistributionFilename() << std::endl;
    }

    inline size_t GetFlowSize() const {
        return this->flow_size_distribution != nullptr ?
            this->flow_size_distribution->GetOne() :
            this->flow_size;
    }
    ...
    inline bool LoopFlows() const {
        return this->loop_flows;
    }

    inline void SetLoopFlows() {
        CHECK(trafficMode == TrafficMode::SHORT, "Loop flow only support short flows.");
        this->loop_flows = true;
        std::cout << "TM[" << this->tm_id << "] loop_flows = " << this->loop_flows << std::endl;
    }
