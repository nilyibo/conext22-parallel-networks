...
void TrafficMatrix::ParseAttributeByKeyValuePair(const std::string &line, bool &binary_demand_matrix, bool &binary_flowsets) {
    ...
    if (std::getline(is_line, key, '=')) {
        if (std::getline(is_line, value)) {
            ...
            } else if (key == GET_VARIABLE_NAME(loop_flows)) {
                ss_value >> value_sst;
                loop_flows = (value_sst != 0);
            ...
            } else if (key == GET_VARIABLE_NAME(flow_size_distribution)) {
                SetFlowSize(0, new CDFDistribution<int>(value));
...
void TrafficMatrix::Serialize(const std::string &outfile) {
    ...
    out << GET_VARIABLE_NAME(loop_flows) << "=" << (loop_flows ? 1 : 0) << std::endl;
    ...
    if (trafficMode == TrafficMode::SHORT || trafficMode == TrafficMode::RPC) {
        ...
        if (flow_size_distribution != nullptr)
            out << GET_VARIABLE_NAME(flow_size_distribution) << "=" << flow_size_distribution->GetDistributionFilename() << std::endl;
    }
    ...
    switch (trafficType) {
        case TrafficType::ALL_TO_ALL: {
            ...
            for (size_t src = lower; src < upper; src++) {
                for (size_t dst = lower; dst < upper; dst++) {
                    if (src == dst) continue;
                    Flow flow(src, dst, this->GetFlowSize());
                    flowsets.push_back(Flowset(absl::StrFormat("tm%zu.%zu->%zu", tm_id, src, dst), flow));
                }
            }
            break;
        ...
        case TrafficType::PERMUTATION: {
            ...
            for (size_t index = 0; index < permutation.size() / 2; index++) {
                ...
                std::vector<Flow> flows({
                    Flow(first, second, this->GetFlowSize()),
                    Flow(second, first, this->GetFlowSize())
                });
        ...
        case TrafficType::MANY_TO_ONE: {
            ...
            for (size_t index = lower; index < upper; index++) {
                Flow flow(index, dst, this->GetFlowSize());
        ...
        case TrafficType::ONE_TO_MANY: {
            ...
            for (size_t index = lower; index < upper; index++) {
                Flow flow(src, index, this->GetFlowSize());
...
void TrafficMatrix::LogFlowSizes(const Flowsets & flowsets) {
    std::cerr << "TM[" << this->GetId() << "] flow size distribution: ";
    for (const auto &flowset: flowsets) {
        for (const auto &flow: flowset.flows) {
            std::cerr << flow.flow_size << ",";
        }
    }
...
TrafficMatrix::~TrafficMatrix() {
    ...
    if (flow_size_distribution != nullptr) delete flow_size_distribution;
}
