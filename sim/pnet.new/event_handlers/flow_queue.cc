...
FlowQueue::FlowQueue(EventList& eventlist, const std::string &name, StopLogger* stop_logger, size_t concurrency, bool loop_flow)
        :EventSource(eventlist, name), stop_logger_(stop_logger), concurrency_(concurrency), loop_flow_(loop_flow) {
    if (loop_flow) {
        CHECK(concurrency == std::numeric_limits<size_t>::max(), "Loop and concurrency cannot be both enabled.");
    }
}
...
void FlowQueue::doNextEvent(EventSource *parent) {
    this->doNextEvent();
    this->RestartFlow(parent);
}
...
void FlowQueue::RestartFlow(EventSource *flow_src) {
     // reset transfers
    auto *tcp_src_transfer = dynamic_cast<TcpSrcTransfer *>(flow_src);
    if (tcp_src_transfer != nullptr) {
        CHECK(tcp_src_transfer->_mSrc == nullptr, "Flow queue should not be attached to individual subflows.");
        tcp_src_transfer->reset(0, 1);
        return;
    }

    auto *dctcp_src_transfer = dynamic_cast<DCTCPSrcTransfer *>(flow_src);
    if (dctcp_src_transfer != nullptr) {
        CHECK(dctcp_src_transfer->_mSrc == nullptr, "Flow queue should not be attached to individual subflows.");
        dctcp_src_transfer->reset(0, 1);
        return;
    }

    auto *multipath_tcp_src = dynamic_cast<MultipathTcpSrc *>(flow_src);
    if (multipath_tcp_src != nullptr) {
        multipath_tcp_src->reset();
        return;
    }

    CHECK(false, "Flow must be one of TcpSrcTransfer, DCTCPSrcTransfer or MultipathTcpSrc!");
}
