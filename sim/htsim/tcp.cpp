...
void
TcpSrc::resetStartTime(simtime_picosec starttime) {
...
TcpSrc::send_packets() {
    ...
    if (!_established){
        ...
        p->set_high_priority(_high_priority);
        _highest_sent = 1;
...
#if 0
    cerr <<"At " << now/(double)1000000000<< " RTO " << _rto/1000000000 << " MDEV "
...
void TcpSrc::doNextEvent() {
    if(_rtx_timeout_pending) {
        ...
        std::cerr << "[TCP] Starting flow " << _name << " at " << timeAsMs(eventlist().now()) << "ms" << std::endl;
        startflow();
    }
}
...
TcpSink::receivePacket(Packet& pkt) {
    ...
    bool high_priority = p->is_high_priority();
    ...
    send_ack(ts,marked,high_priority);
}

void
TcpSink::send_ack(simtime_picosec ts,bool marked,bool high_priority) {
    ...
    ack->set_ts(ts);
    ack->set_high_priority(high_priority);
    if (marked)
...
void
TcpRtxTimerScanner::deregisterTcp(TcpSrc &tcpsrc) {
    _tcps.remove(&tcpsrc);
}

void
TcpRtxTimerScanner::deregisterAllTcps() {
    _tcps.clear();
}
