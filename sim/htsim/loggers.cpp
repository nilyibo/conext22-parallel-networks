...
MultipathTcpLoggerSampling::MultipathTcpLoggerSampling(simtime_picosec period, EventList& eventlist,
                                                       MultipathTcpLogger::MultipathTcpEvent event_type):
    EventSource(eventlist, "MultipathTcpLoggerSampling"), _period(period), _event_type(event_type)
{
    eventlist.sourceIsPendingRel(*this,0);
}

void MultipathTcpLoggerSampling::doNextEvent() {
    eventlist().sourceIsPendingRel(*this,_period);
    for (auto src: _srcs) {
        logMultipathTcpEvent(*src, _event_type);
    }
}

void MultipathTcpLoggerSampling::monitorMultipathTcpSource(MultipathTcpSrc* src) {
    _srcs.push_back(src);
}

void MultipathTcpLoggerSampling::logMultipathTcpEvent(MultipathTcpSrc& mtcp, 
                                               MultipathTcpLogger::MultipathTcpEvent ev){
    if (ev==MultipathTcpLogger::CHANGE_A) {
        _logfile->writeRecord(MultipathTcpLogger::MTCP, mtcp.id,
                              ev, mtcp.a, mtcp._alfa, 0);
    } else if (ev==MultipathTcpLogger::RTT_UPDATE) {
