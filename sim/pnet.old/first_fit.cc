...
FirstFit::FirstFit(simtime_picosec scanPeriod, EventList& eventlist, size_t host_link_speed_mbps, vector<const Route*>*** n) : EventSource(eventlist,"FirstFit"), _scanPeriod(scanPeriod) /*, _init(0)*/
{
    eventlist.sourceIsPendingRel(*this, _scanPeriod);
    net_paths = n;

    threshold = (int)(timeAsSec(_scanPeriod) * host_link_speed_mbps * 100);
...
void FirstFit::run(){
    cerr << "Running FirstFit at time " << timeAsMs(eventlist().now()) << "ms. ";
    cerr << "Scanning " << flow_counters.size() << " flows..." << std::endl;
